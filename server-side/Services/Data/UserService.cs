...
namespace Services.Data
{
  public class UserService : IUserService
  {
    ...
    public async Task CreateAsync(User newUser, UserRole role)
    {
      ...
      if (success)
      {
        if (newUser.ConfirmToken != null)
        {
          ...
          string title = "Registered account";
          bool btn = true;
    ...
    public async Task<UserDTO> UpdateAsync(User userToBeUpdated, User user)
    {
      ...
      userToBeUpdated.Fullname = user.Fullname ?? userToBeUpdated.Fullname;
      userToBeUpdated.Slug = user.Slug == null ? user.Fullname.Replace(" ", "-").ToLower() : userToBeUpdated.Slug;
      userToBeUpdated.Email = user.Email ?? userToBeUpdated.Email;
      userToBeUpdated.Birth = user.Birth.Year != 0001 ? user.Birth : userToBeUpdated.Birth;
      userToBeUpdated.Phone = user.Phone ?? userToBeUpdated.Phone;
      userToBeUpdated.Password = user.Password != null ? Crypto.HashPassword(user.Password) : userToBeUpdated.Password;

      userToBeUpdated.Biography = user.Biography ?? userToBeUpdated.Biography;
      userToBeUpdated.PostalCode = user.PostalCode ?? userToBeUpdated.PostalCode;
      userToBeUpdated.Address = user.Address ?? userToBeUpdated.Address;
      userToBeUpdated.City = user.City ?? userToBeUpdated.City;
      userToBeUpdated.State = user.State ?? userToBeUpdated.State;
      userToBeUpdated.Country = user.Country ?? userToBeUpdated.Country;
      ...
      userToBeUpdated.ConnectionId = user.ConnectionId ?? null;
    ...
    public async Task<UserDTO> InviteTokenAsync(User user)
    {
      ...
      if (success)
      {
        if (user.InviteToken != null)
        {
          ...
          string purpose = "Password reset process";
          string title = "'s password reset";
          string body = "To complete reset process click to the button!";
          bool btn = true;
